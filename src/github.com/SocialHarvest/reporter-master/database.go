...
type SocialHarvestDB struct {
	Postgres *sqlx.DB
	//InfluxDB *influxdatabase.Client
	Series   []string
	Schema   struct {
...
func newDatabase(c config.SocialHarvestConf) *SocialHarvestDB {
	...
	case "postgres", "postgresql":
		...
		database.Postgres, err = sqlx.Connect("postgres", "host="+c.Database.Host+" port="+strconv.Itoa(c.Database.Port)+" sslmode=disable dbname=harvester user=admin password=bannon39")
		if err != nil {
			log.Println(err)
			return &database
		}
...
func (db *SocialHarvestDB) FieldCounts(queryParams CommonQueryParams, fields []string, extraParams map[string]string) ([]ResultAggregateFields, ResultCount) {
	...
	if database.Postgres != nil {
		...
		buffer.Reset()
		err = database.Postgres.Get(&total, tQuery)
		if err != nil {
			log.Println(err)
		}
		...
		for _, field := range fields {
			if len(field) > 0 {
				...
				err = database.Postgres.Select(&valueCounts, query)
				if err != nil {
					log.Println(err)
					continue
				}
				...
				var dC int
				err = database.Postgres.Get(&dC, query)
				if err != nil {
					log.Println(err)
				}
...
func (database *SocialHarvestDB) Count(queryParams CommonQueryParams, fieldValue string) ResultCount {
	...
	if database.Postgres != nil {
		...
		if fieldValue != "" && sanitizedQueryParams.Network == "" {
			err = database.Postgres.Get(&count, query, fieldValue)
		} else if fieldValue != "" && sanitizedQueryParams.Network != "" {
			err = database.Postgres.Get(&count, query, fieldValue, sanitizedQueryParams.Network)
		} else if fieldValue == "" && sanitizedQueryParams.Network != "" {
			err = database.Postgres.Get(&count, query, sanitizedQueryParams.Network)
		} else {
			err = database.Postgres.Get(&count, query)
		}
...
func (database *SocialHarvestDB) Messages(queryParams CommonQueryParams, conds BasicConditions) ([]config.SocialHarvestMessage, uint64, uint64, uint64) {
	...
	if database.Postgres != nil {
		var rows *sqlx.Rows
		rows, err = database.Postgres.Queryx(query)
		if err != nil {
			log.Println(err)
			return results, 0, sanitizedQueryParams.Skip, sanitizedQueryParams.Limit
		}
		...
		err = database.Postgres.Get(&total, countQuery)
		if err != nil {
			log.Println(err)
		}
	}
